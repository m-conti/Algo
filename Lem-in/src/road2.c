...
/*   Updated: 2018/06/02 17:34:40 by tbehra           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_queue		*trace_roads(t_anthill *ant, t_queue *queue, t_road *all_roads)
{
	...
	{
		if (ant->hill[queue->room].links[i] != ant->start && (!ant->hill[queue->room].links[i] || ant->path[ant->hill[queue->room].links[i]] < ant->path[ant->hill[queue->room]]))
	}
}
