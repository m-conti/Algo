...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/20 14:19:24 by tbehra            #+#    #+#             */
/*   Updated: 2018/05/21 21:51:25 by tbehra           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int16_t	fill_frontier_test(t_filler *f, int x, int y)
{
	...
	if (f->tab[y][x] != '.')
		f->frontier[y][x] = -3;
	else if ((x - f->piece.x) >= 0 && (x - f->piece.x) < f->piece.xmax && (y - f->piece.y) >= 0 &&
		(y - f->piece.y) < f->piece.ymax && f->piece.tab[(y - f->piece.y)][(x - f->piece.x)] == '*')
		f->frontier[y][x] = 3;
	else
